...
export class EntraIdApi extends RESTDataSource {
  ...
  async getEmployeeId(entraIdUserObjectId) {
    ...
    try {
      ...
      this.logger.verbose('#datasource - entra - get the employee ID for the user', {
        entraIdUserObjectId,
        code: ENTRA_REQUEST_EMPLOYEE_LOOKUP_001
      })
      const response = await this.get(`v1.0/users/${entraIdUserObjectId}?$select=employeeId`, {
        headers: {
          Authorization: token
        },
        cacheOptions: {
      ...
      this.logger.http('#datasource - entra - Successful get employee ID for user', {
        code: ENTRA_REQUEST_EMPLOYEE_LOOKUP_001,
        requestTimeMs
      })
    } catch (error) {
      this.logger.error('#datasource - entra - Could not get the employee ID for the user', {
        entraIdUserObjectId,
        error,
        code: ENTRA_REQUEST_EMPLOYEE_LOOKUP_001
      })
